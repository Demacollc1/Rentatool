...
async function cargar() {
  try {
    ...
    if (d.contact) {
      entrega += ' · Responsable: ' + d.contact.name +
          (d.contact.role ? ' (' + d.contact.role + ')' : '');
    }
    $('entrega').textContent = entrega;
    ...
    $('cargando').innerHTML = (e.message || '').includes('no encontrado')
        ? '<b>Este contrato aún no está disponible.</b><br>' +
          'Pide al personal de Alivio Constructor que presione ' +
          'sincronizar en su app y vuelve a escanear el código.'
        : 'No se pudo cargar el contrato: ' + e.message +
          '<br>Revisa tu conexión e intenta de nuevo.';
  }
}
